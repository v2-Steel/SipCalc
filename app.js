...
$('.keys').click(function () {
    ...
    if (!action) {
        if($('#gridContainer').data('previouskeytype') == "graph"){
...
document.getElementById("copulaInput").addEventListener("change", () => {
    console.log("copulaInput")

    var file_to_read = document.getElementById("copulaInput").files[0];
    var fileread = new FileReader();
    fileread.onload = function (e) {
        var str = e.target.result;

        // parsing the JSON
        var libraryJson = JSON.parse(str);
        ...
        libraryJson.sips.forEach((sip, i) => {
            ...
            // Keep the original ref structure - don't change source to "rng"
            ...
            const input = document.getElementById('fileInput' + (i+1)); // replace with your actual input
